...
struct t_joueur *cartechance(struct t_joueur *p, int nb,int i) // carte chance
{
    ...
    gotoligcol(12, 115);
    printf("Carte Chance ! ");
    r=rand() % 16+1;    //génère un nombre aléatoire compris entre 1 et 16


        if (r==1){
            gotoligcol(15, 115);
            printf("Une amende.  Versez 150 euros a la banque.");
            (p)[i].argent -= 150;   //Permet de modifier la valeur "argent" du joueur (paranthèses sur le p pour tab de struct)
        }
        if (r==2){
            gotoligcol(15, 115);
